...
{
	LOG(LOG_INFO,_("RenderThread start=") << data);
	status=STARTED;
	engineData=data;
...
{
	LOG(LOG_INFO,_("RenderThread stop"));
	initialized.signal();
}

RenderThread::~RenderThread()
{
	LOG(LOG_INFO,_("enter ~RenderThread this=") << this);
	wait();
	LOG(LOG_INFO,_("~RenderThread this=") << this);
}
...
void RenderThread::handleUpload()
{
	...
	prevUploadJob=u;
#else
    LOG(LOG_INFO, "RenderThread::handleUpload not implement");
#endif
}
...
{
    LOG(LOG_INFO, "" << __PRETTY_FUNCTION__);
...
#elif !defined(ENABLE_GLES2) && !defined(ENABLE_DUMMYGL)
...
#elif !defined(ENABLE_DUMMYGL) //egl
	mDisplay = XOpenDisplay(NULL);
	int a;
...
{
	LOG(LOG_INFO,_("RenderThread::worker this=") << this);
	setTLSSys(m_sys);
	...
	try
	{
		...
		while(1)
		{
	        LOG(LOG_INFO,_("event.wait RenderThread::worker this=") << this);
			event.wait();
...
{
	LOG(LOG_INFO,_("RenderThread::deinit this=") << this);
#ifndef ENABLE_DUMMYGL
...
{
#ifdef ENABLE_DUMMYGL
    LOG(LOG_NOT_IMPLEMENTED," couldn't loadshaderprograms");
    return true;
#else
...
{
#ifdef ENABLE_DUMMYGL
	LOG(LOG_NOT_IMPLEMENTED,"commonGLDeinit");
    return;
#else
...
{
#ifdef ENABLE_DUMMYGL
    LOG(LOG_INFO,_("RenderThread::commonGLInit not implement: ") << width << "x" << height);
#else
	GLenum err;
...
{
#ifdef ENABLE_DUMMYGL
    LOG(LOG_INFO, "renderText: " << text << "(" << x << "," << y << ")");
    return;
#else
...
{
#ifdef ENABLE_DUMMYGL
    LOG(LOG_NOT_IMPLEMENTED," mapCairoTexture");
    return;
#else
	glEnable(GL_TEXTURE_2D);
...
void RenderThread::renderErrorPage(RenderThread *th, bool standalone)
{
	...
	LOG(LOG_INFO, "We're sorry, Lightspark encountered a yet unsupported Flash file" << errorMsg);
	errorMsg.str("");
...
{
#ifdef ENABLE_DUMMYGL
    LOG(LOG_INFO,_("RenderThread::allocateNewGLTexture not implement"));
    return 0;
#else
