...
//	gtk_init (&argc, &argv);
	...
	for(int i=1;i<argc;i++)
	{
        ...
        else if(strcmp(argv[i],"--flashvars")==0)
        {
            ...
            flashVars=argv[i];
            LOG(LOG_INFO, "flashVars: " << flashVars);
        }
		else
		{
	...
	if(fileName==NULL)
	{
		LOG(LOG_ERROR, "Usage: " << argv[0] << " [--url|-u http://loader.url/file.swf]" <<
			...
			" [--exit-on-error] [--HTTP-cookies cookie] [--air] [--flashvars]" <<
#ifdef PROFILING_SUPPORT
    ...
    if(flashVars)
        sys->parseParametersFromFlashvars(flashVars);
#ifdef PROFILING_SUPPORT
