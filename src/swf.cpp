...
void SystemState::startRenderTicks()
{
	assert(renderThread);
	assert(renderRate);
#if 0 //liangtao01 Disable RenderThread::tick()
	removeJob(renderThread);
...
{
    LOG(LOG_INFO, "SystemState::delayedCreation enter");
	gdk_threads_enter();
...
void SystemState::createEngines()
{
    ...
    LOG(LOG_INFO, "SystemState::delayedCreation < runInGtkThread");
#if 0 //liangtao01 remove renderthread
	engineData->runInGtkThread(sigc::mem_fun(this, &SystemState::delayedCreation));
    LOG(LOG_INFO, "SystemState::delayedCreation < waitForInitialization");
...
#endif
    LOG(LOG_INFO, "SystemState::delayedCreation < waitForInitialization leave");
